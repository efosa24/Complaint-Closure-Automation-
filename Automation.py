...
# Load the data
df_filtered = pd.read_csv('path_to__data.csv')
...
# Sort the data
Sorted_data = APAC_Data.sort_values(by=['tracking_code', 'Priority'])

# Add duplicate column
Sorted_data['Duplicate'] = Sorted_data['tracking_code'].eq(Sorted_data['tracking_code'].shift()) & \
                     Sorted_data['tracking_code'].eq(Sorted_data['tracking_code'].shift(-1))
Sorted_data['Duplicate'] = Sorted_data['Dup'].replace({True: 'dup', False: ''})

# Drop rows where Family level 2 description are Exuviance, Maui Moisture, and OGX
Sorted_data = Sorted_data[~Sorted_data['Brand'].isin(['Exu', 'Mauisture', 'OGXES'])]

# Drop rows where Dup is 'dup'
Sorted_data = Sorted_data[Sorted_data['Duplicate'] != 'dup']

# Drop rows where category description is 'medical/Lack of effect'
Sorted_data = Sorted_data[~Sorted_data['cat_desc'].str.contains('medical/Lack', case=False, na=False)]

# Pivot table
df_pivot = Sorted_data.groupby(['Country', 'Late']).size().unstack(fill_value=0).reset_index()
...
Sorted_data = Sorted_data[['tracking_code', 'Duplicate', 'owner_grp', 'seriousness', 'reg_class', 'prod_fmly_nm',
...
# Sort data by tracking code (Assuming EMEA_Seriousness is a function you need to define)
# EMEA_Data = EMEA_Seriousness(EMEA_Data)

# Sort the data
Sorted_data1 = EMEA_Data.sort_values(by=['tracking_code', 'Priority'])

# Add duplicate column
Sorted_data1['Duplicate'] = Sorted_data1['tracking_code'].eq(Sorted_data1['tracking_code'].shift()) & \
                      Sorted_data1['tracking_code'].eq(Sorted_data1['tracking_code'].shift(-1))
Sorted_data1['Duplicate'] = Sorted_data1['Duplicate'].replace({True: 'dup', False: ''})

# Drop rows where Family level 2 description are Exuviance, Maui Moisture, and OGX
Sorted_data1 = Sorted_data1[~Sorted_data1['Brand'].isin(['Exu', 'Moisture', 'GXXXX'])]

# Drop rows where Dup is 'dup'
Sorted_data1 = Sorted_data1[Sorted_data1['Duplicate'] != 'dup']

# Drop rows where category description is 'medical/Lack of effect'
Sorted_data1 = Sorted_data1[~Sorted_data1['cat_desc'].str.contains('medical/Lack of effect', case=False, na=False)]

# Pivot table
df_pivot1 = Sorted_data1.groupby(['Country', 'Late']).size().unstack(fill_value=0).reset_index()
...
# Select relevant columns
Sorted_data1 = Sorted_data1[['tracking_code', 'Duplicate', 'owner_grp', 'seriousness', 'reg_class', 'prod_fmly_nm',
                             'issue_status', 'issue_age', 'Late', 'iss_stat_esig_id', 'jnj_aware_dt',
                             'cat_desc', 'iss_cls_dt', 'cyc_time_init', 'iss_entrd_gcc', 'issue_from',
...
# Add duplicate column
Sorted_data2['Duplicate'] = Sorted_data2['tracking_code'].eq(Sorted_data2['tracking_code'].shift()) & \
                      Sorted_data2['tracking_code'].eq(Sorted_data2['tracking_code'].shift(-1))
Sorted_data2['Duplicate'] = Sorted_data2['Dup'].replace({True: 'dup', False: ''})

# Drop rows where Family level 2 description are Exuviance, Maui Moisture, and OGX
Sorted_data2 = Sorted_data2[~Sorted_data2['Brand'].isin(['Exu', 'Moisture', 'GXXXXXXX'])]

# Drop rows where Dup is 'dup'
Sorted_data2 = Sorted_data2[Sorted_data2['Duplicate'] != 'dup']

# Drop rows where category description is 'medical/Lack of effect'
Sorted_data2 = Sorted_data2[~Sorted_data2['cat_desc'].str.contains('medical/Lack of effect', case=False, na=False)]

# Pivot table
df_pivot2 = Sorted_data2.groupby(['Country', 'Late']).size().unstack(fill_value=0).reset_index()
